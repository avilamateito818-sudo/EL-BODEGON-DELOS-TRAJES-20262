...
const FILE_PATH = 'data/mensajes.json';
...
module.exports = async (req, res) => {
  ...
  const waLink = `https://wa.me/${CONTACT_PHONE}?text=${encodeURIComponent(
    `Mensaje del sitio (${record.tipo}): ${record.mensaje ? record.mensaje + '. ' : ''}Nombre: ${record.nombre}. Correo/WhatsApp: ${record.correo || record.whatsapp}`
  )}`;
  ...
  try {
    ...
    const body = {
      message: 'Nuevo mensaje del formulario de contacto',
